...
plt.xlabel("Převýšení [m]")
plt.ylabel("Vzdálenost [km]")
plt.show()

# 7) kolik se udelalo kroku v prosinci?
# a) pomoci for-cyklu
kroky_prosinec = 0
for k in range(len(prehled)):
    if prehled.loc[k,'Měsíc'] == 'prosinec':
        kroky_prosinec += prehled.loc[k, 'Kroky']

print('Celkem se ušlo',kroky_prosinec, 'kroků.')

# b) pomoci pandas
kroky_prosinec2 = prehled.loc[prehled['Měsíc']=='prosinec', 'Kroky'].sum()
